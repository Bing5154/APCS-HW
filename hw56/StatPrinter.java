...
public class StatPrinter {
	...
	//create an arrayList (_frequency) with slots from 0 to integer max inclusive
	while(_frequency.size() <= max(data)) {
	    _frequency.add(0); //placeholder
	}
	//after populating _frequency with placeholder
	//loop through each element of _frequency
	//use _frequency.get(i) to return the frequency of i in data
	//set i to the frequency of it in data
	for (int i : data) {
    ...
    {
	//check if the element at index i (param) is a local mode or not
	...
	//local mode exludes first and last element
    ...
    //postcond: print out the histogram
    public void printHistogram( int longestBar ) 
    {
	int ratio = longestBar / max(_frequency);
	//create a row / bar for each element
	for (int i = 0; i < _frequency.size(); i++) {
	    String bar = ""; //container
	    //standardize star output by multiplying freq at each index by ratio
	    for(int j = _frequency.get(i) * ratio; j > 0; j--) {
		bar += "*";
	    }
